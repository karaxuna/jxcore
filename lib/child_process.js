...
function setupChannel(target, channel) {
  ...
  target.send = function(message, handle) {
    ...
    if (handle) {
      ...
      handle = obj.send.apply(target, arguments);
...
exports.fork = function(modulePath /* , args, options */) {
  ...
  // for native packages fork() can work without problem if modulePath is not
  // given (it just forks itself, which is used by cluster.fork() )
  if (process.isPackaged && modulePath) {
    // but if modulePath is given, user expects this modulePath to be forked
    // and that case we don't support yet
    if (!options || !options.execPath)
      throw new Error(
          'Native packages cannot fork specific modules. ' +
          'If you have jx binary installed, you may provide it with ' +
          '`options.execPath`.');
  }

  // Prepare arguments for fork:
  execArgv = options.execArgv || process.execArgv;
  args = execArgv.concat(modulePath ? [modulePath] : [], args);
...
exports._forkChild = function(fd) {
  // preload tcp_wrap since we may receive a message immediately
...
var spawn = exports.spawn = function(file /* , args, options */) {
  ...
  var envPairs = [];
  for (var key in env) {
    if (typeof env.hasOwnProperty !== 'function' || env.hasOwnProperty(key) ||
        env.__proto__[key])
...
ChildProcess.prototype.kill = function(sig) {
  ...
  if (signal === undefined) { throw new Error('Unknown signal: ' + sig); }
...
function _validateStdio(stdio, sync) {
  ...
  } else if (!(stdio instanceof Array)) {
...
function normalizeSpawnArguments(file /*, args, options*/) {
  ...
  if (arguments[1] instanceof Array) {
    args = arguments[1].slice(0);
    options = arguments[2];
  } else if (arguments[1] !== undefined && (typeof arguments[1] !== 'object' || arguments[1] === null)) {
    throw new TypeError('Incorrect value of args option');
  } else {
    args = [];
    options = arguments[1];
