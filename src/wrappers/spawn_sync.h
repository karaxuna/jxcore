...
namespace node {
  ...
  public:
    ...
    JS_LOCAL_OBJECT GetOutputAsBuffer() const;
  ...
  private:
    ...
    explicit SyncProcessRunner();
    ~SyncProcessRunner();

    JS_LOCAL_OBJECT Run(JS_LOCAL_VALUE options, jxcore::PArguments args);
    void TryInitializeAndRunLoop(JS_LOCAL_VALUE options);
    ...
    JS_LOCAL_ARRAY BuildResultObject(jxcore::PArguments args);
    JS_LOCAL_ARRAY BuildOutputArray(jxcore::PArguments args);

    int ParseOptions(JS_LOCAL_VALUE js_value);
    int ParseStdioOptions(JS_LOCAL_VALUE js_value);
    int ParseStdioOption(int child_fd, JS_LOCAL_OBJECT js_stdio_option);
    ...
    static bool IsSet(JS_LOCAL_VALUE value);
    template <typename t> static bool CheckRange(JS_LOCAL_VALUE js_value);
    int CopyJsString(JS_LOCAL_VALUE js_value, const char** target);
    int CopyJsStringArray(JS_LOCAL_VALUE js_value, char** target);
